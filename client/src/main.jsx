...
import SessionManager from '../slices/sessionManager.jsx'
...
function ChatBox({ onClose, event, id, register }) {
    ...
    useEffect(() => {
        const fetchChatData = async () => {
            try {
                ...
                const usernamesResponse = await axios.post('http://127.0.0.1:8000/userName', {
                    senderIds: [...senderIds],
                });
        ...
        try {
            ...
            if (response.data.error) {
                ...
                socket.emit('send_message', {
                    ...
                    eventId: id,
                    content:
                        response.data.chat.messages[response.data.chat.messages.length - 1].content,
    ...
    useEffect(() => {
        const handleReceiveMessage = async (data) => {
            setMessage((list) => [
                ...list,
                { senderId: data.senderId, content: data.content, timestamp: data.timestamp },
            ]);

            console.log(`Received message from ${data.senderId}: ${data.content}`);

            const newUsernameResponse = await axios.post('http://127.0.0.1:8000/userName', {
        ...
        <div className="chat-box">
            ...
            <div className="chat__body">
                {message.map((message2) => (
                    <p
                        key={message2._id}
                        className={`chat__message ${message2.senderId === user.userId && 'chat__receiver'
                            }`}
                    >
                        <span className="chat__name">{usernames[message2.senderId] || 'Unknown User'}</span>
                        {message2.content}
        ...
        <>
            <div className="event-container">
                <div className="event-details">
                    <img className="event-image" src={getImageUrl()} alt={event.title} />
                            ...
                            <div className="mid">
                                ...
                                <div className="notification-badge">
                                    ...
                                    <ChatBubbleOutlineOutlinedIcon
                                        onClick={() => handleChatIconClick(event_id)}
                                        className="chat-icon"
                                    />
                                </div>
                            </div>
